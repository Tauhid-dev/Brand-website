...
import { BillingCheckoutButton } from "@/components/BillingProviderControls";
...
export default async function AccountPage() {
  ...
  return <PortalShell kind="Customer" title={`Welcome, ${account.customer.contactName}`} subtitle={`${account.customer.businessName} · ${account.customer.externalReference}`} user={principal.email}>
    ...
    <Panel id="billing" title="Billing"><dl className="portal-details"><div><dt>Payment state</dt><dd><Status value={billing.paymentState} /></dd></div><div><dt>Entitlement state</dt><dd><Status value={billing.entitlementState} /></dd></div><div><dt>Effective recurring price</dt><dd>{billing.pricing ? new Intl.NumberFormat("en-AU", { style: "currency", currency: billing.pricing.currency }).format(billing.pricing.effectivePriceMinor / 100) : "—"}</dd></div><div><dt>Renewal / cancellation date</dt><dd>{billing.subscription?.cancelAt?.toLocaleDateString("en-AU") ?? billing.subscription?.currentPeriodEnd?.toLocaleDateString("en-AU") ?? "—"}</dd></div><div><dt>Billing contact</dt><dd>{billing.profile ? `${billing.profile.contactName} · ${billing.profile.contactEmail.value}` : "Uses the primary account contact"}</dd></div></dl>{["PENDING", "TRIAL"].includes(account.subscription?.status ?? "") ? <BillingCheckoutButton /> : null}<DataTable rows={account.invoices} columns={[["invoiceNumber", "Invoice"], ["status", "Status"], ["amountDueMinor", "Amount due (minor units)"], ["dueAt", "Due"], ["paidAt", "Paid"]]} /><DataTable rows={account.reminders} columns={[["invoiceNumber", "Invoice"], ["stage", "Stage"], ["status", "Reminder status"], ["scheduledFor", "Scheduled"]]} /></Panel>
