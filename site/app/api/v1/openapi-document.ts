...
const responseSchemas = Object.fromEntries(["PublicPlan", "CustomerAccount", "Customer", "BillingOverview", "Plan", "Offering", "PlanPrice", "Discount", "PromotionCode", "Subscription", "AuditEvent", "OperationResult", "BillingCheckout", "ProviderSubscriptionSync", "AgentCustomer", "AgentBootstrap", "AgentLink", "AgentReconciliation", "AgentProcessingResult", "ProvisioningJob", "PromotionValidation", "BillingProfile", "BillingNote", "PriceOverride", "CustomerDiscount", "ServiceCredentialIssue", "EntitlementValidation", "ProductionReadiness", "SystemMaintenanceResult"].map((name) => [`${name}Response`, { type: "object", additionalProperties: false, required: ["data"], properties: { data: schemaRef(name) } }])) as Record<string, unknown>;
...
export const openApiDocument = {
  ...
  paths: {
    ...
    "/customer/billing/checkout": { post: { tags: ["Customer"], security: sessionSecurity, summary: "Initiate an idempotent Stripe subscription checkout from the current contracted price", parameters: [idempotency], requestBody: jsonBody("EmptyRequest"), responses: { ...created("BillingCheckout"), "503": errorResponse("Outbound billing is not configured or live billing is not approved") } } },
    ...
    "/admin/subscriptions/{subscriptionId}/provider-sync": { post: { tags: ["Admin"], security: sessionSecurity, summary: "Synchronize an explicit operation to the configured billing provider", parameters: [pathId("subscriptionId"), idempotency], requestBody: jsonBody("ProviderSubscriptionSyncRequest"), responses: { ...ok("ProviderSubscriptionSync"), "503": errorResponse("Outbound billing is not configured or live billing is not approved") } } },
    ...
    "/admin/system/readiness": { get: { tags: ["Admin"], security: sessionSecurity, summary: "Get a field-minimised production readiness projection", responses: ok("ProductionReadiness") } },
    "/admin/system/maintenance": { post: { tags: ["Admin"], security: sessionSecurity, summary: "Run bounded retention cleanup and billing-webhook recovery", parameters: [idempotency], requestBody: jsonBody("EmptyRequest"), responses: ok("SystemMaintenanceResult") } },
    ...
    "/integrations/agent/customers/{customerId}/subscription-validation": serviceGet("Validate the subscription used by the agent runtime", ["subscription:validate", "entitlement:read"], "EntitlementValidation"),
    ...
    "/integrations/agent/customers/{customerId}/agent-link": { patch: { tags: ["Agent integration"], security: serviceSecurity(["agent-link:write"]), summary: "Synchronize the external agent link", parameters: [pathId("customerId"), idempotency], requestBody: jsonBody("AgentLinkRequest"), responses: ok("AgentLink") } },
    ...
    "/integrations/agent/customers/{customerId}/reconciliation": { post: { tags: ["Agent integration"], security: serviceSecurity(["agent-link:write"]), summary: "Reconcile commercial, link and external agent state", parameters: [pathId("customerId"), idempotency], requestBody: jsonBody("AgentReconciliationRequest"), responses: { ...ok("AgentReconciliation"), "503": errorResponse("Agent platform not configured") } } },
    "/integrations/agent/provisioning-jobs/process": { post: { tags: ["Agent integration"], security: serviceSecurity(["agent-link:write"]), summary: "Process one ready provisioning job using the configured adapter", responses: { ...ok("AgentProcessingResult"), "503": errorResponse("Agent platform not configured") } } },
