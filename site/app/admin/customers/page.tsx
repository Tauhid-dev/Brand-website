...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<{ q?: string; subscription?: string }> }) {
  ...
  return <PortalShell kind="Admin" title="Customers" subtitle="Search customer, contact and subscription records." user={principal.displayName}>
    <Panel title="Customer search"><form className="portal-search"><label><span>Search</span><input name="q" defaultValue={filters.q} placeholder="Business, email, phone, ID or reference" /></label><label><span>Subscription</span><select name="subscription" defaultValue={filters.subscription ?? ""}><option value="">All</option>{["PENDING", "TRIAL", "ACTIVE", "PAST_DUE", "SUSPENDED", "CANCEL_AT_PERIOD_END", "CANCELLED"].map((status) => <option key={status}>{status}</option>)}</select></label><button className="button small">Search</button></form></Panel>
