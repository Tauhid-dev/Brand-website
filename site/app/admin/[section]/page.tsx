...
function sections(read: Awaited<ReturnType<typeof portalReadRepository>>): Record<string, Section> {
  return {
    ...
    billing: { title: "Billing", subtitle: "Invoice history and payment-reminder operations.", permission: "BILLING_READ", load: async () => { const data = await read.getBilling(); return [{ title: "Invoices", rows: data.invoices, columns: [["invoiceNumber", "Invoice"], ["customerName", "Customer"], ["status", "Status"], ["totalMinor", "Total"], ["dueAt", "Due"]] }, { title: "Payment reminders", rows: data.reminders, columns: [["invoiceNumber", "Invoice"], ["customerName", "Customer"], ["status", "Status"], ["scheduledFor", "Scheduled"]] }]; } },
