...
test("production worker serves critical journeys and every homepage navigation target", async () => {
  ...
  const paths = [...new Set([...html.matchAll(/href="([^"#]+)"/g)].map((match) => match[1]).filter((href) => href.startsWith("/") && !href.startsWith("//") && !href.startsWith("/api/") && !href.startsWith("/assets/")))];
...
test("production HTML and render-critical assets remain within launch budgets", async () => {
  ...
  const assets = await files(new URL("../dist/client/assets/", import.meta.url));
  const javascriptBytes = await totalBytes(assets.filter((path) => path.pathname.endsWith(".js")));
  const cssBytes = await totalBytes(assets.filter((path) => path.pathname.endsWith(".css")));
  assert.ok(javascriptBytes < 350_000, `JavaScript assets exceed 350 KB (${javascriptBytes})`);
  assert.ok(cssBytes < 50_000, `CSS assets exceed 50 KB (${cssBytes})`);
...
async function totalBytes(paths) { return (await Promise.all(paths.map(async (path) => (await stat(path)).size))).reduce((total, size) => total + size, 0); }
