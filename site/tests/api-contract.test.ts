...
test("OpenAPI inventories every purpose-specific surface and security scheme", () => { assert.equal(openApiDocument.openapi, "3.1.0"); const paths = Object.keys(openApiDocument.paths); for (const prefix of ["/public/", "/customer/", "/admin/", "/integrations/agent/"]) assert.equal(paths.some((path) => path.startsWith(prefix)), true); assert.equal(paths.includes("/webhooks/billing/{provider}"), true); assert.deepEqual(Object.keys(openApiDocument.components.securitySchemes).sort(), ["ChatGPTSession", "ServiceBearer", "StripeSignature"]); assert.ok(openApiDocument.paths["/admin/customers"]); assert.ok(openApiDocument.paths["/integrations/agent/customers/{customerId}/bootstrap"]); });
