...
test("billing account, invoice history, lifecycle, and reminders remain provider-neutral", async () => {
  const context = setup();
  const account = await new LinkBillingAccountService(context.repository, context.references, context.ids, context.clock, NOOP_AUDIT).execute({
  ...
  const invoice = await new CreateInvoiceService(
    context.repository, new D1SubscriptionRepository(context.database), context.references, context.ids, context.clock, NOOP_AUDIT,
  ).execute({
  ...
  let now = NOW;
  const lifecycle = new InvoiceLifecycleService(context.repository, { now: () => now }, NOOP_AUDIT);
  ...
  const schedule = new SchedulePaymentReminderService(context.repository, context.ids, { now: () => now }, NOOP_AUDIT);
  ...
  now = dueAt;
  const sent = await new RecordPaymentReminderOutcomeService(context.repository, { now: () => now }, NOOP_AUDIT).execute(reminder, { sent: true });
