...
import {
  adminUserRoles, adminUsers, agentLinks, agentProvisioningAttempts, agentProvisioningJobs, auditEvents,
...
const CURRENT_SUBSCRIPTION_STATUSES = ["PENDING", "TRIAL", "ACTIVE", "PAST_DUE", "SUSPENDED", "CANCEL_AT_PERIOD_END"];
...
export class D1PortalReadRepository implements PortalReadRepository {
  ...
  async getAgents() { const [links, jobs, attempts] = await Promise.all([this.db.select({ link: agentLinks, customerName: customers.businessName }).from(agentLinks).innerJoin(customers, eq(customers.id, agentLinks.customerId)).orderBy(desc(agentLinks.updatedAt)), this.db.select({ job: agentProvisioningJobs, customerName: customers.businessName, platform: agentLinks.agentPlatform }).from(agentProvisioningJobs).innerJoin(customers, eq(customers.id, agentProvisioningJobs.customerId)).innerJoin(agentLinks, eq(agentLinks.id, agentProvisioningJobs.agentLinkId)).orderBy(desc(agentProvisioningJobs.createdAt)).limit(200), this.db.select({ attempt: agentProvisioningAttempts, operation: agentProvisioningJobs.operation, customerName: customers.businessName }).from(agentProvisioningAttempts).innerJoin(agentProvisioningJobs, eq(agentProvisioningJobs.id, agentProvisioningAttempts.jobId)).innerJoin(customers, eq(customers.id, agentProvisioningJobs.customerId)).orderBy(desc(agentProvisioningAttempts.createdAt)).limit(200)]); return { links: links.map((row) => ({ ...row.link, customerName: row.customerName })), jobs: jobs.map((row) => ({ ...row.job, customerName: row.customerName, platform: row.platform })), attempts: attempts.map((row) => ({ ...row.attempt, operation: row.operation, customerName: row.customerName })) }; }
